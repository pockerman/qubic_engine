...
class IterateTask
{
   public:
        ...
        IterateTask(const parframe::range1d<iterator_t>& range, Args&... args);

        /// \brief Execute the task
        void operator()();

        /// \brief Set the range to work on
        void set_range(const parframe::range1d<iterator_t>& range){ range_ = range;}
...
private:
    ...
    /// do work on
    parframe::range1d<iterator_t> range_;
...
template<typename IteratorTp, typename OpTp>
template<typename... Args>
IterateTask<IteratorTp, OpTp>::IterateTask(const parframe::range1d<iterator_t>& range, Args&... args)
    :
  range_(range),
  op_(args...)
{}
...
public:
    ...
    /// operation requires to execute
    template<typename...Args>
    IterateTaskWithResult(const parframe::range1d<iterator_t>& range, Args&... args);

    /// \brief Execute the task
    void operator()(){task_();}

    /// \brief Set the range to work on
    void set_range(const parframe::range1d<iterator_t>& range){task_.set_range(range);}
...
template<typename IteratorTp, typename OpTp>
template<typename...Args>
IterateTaskWithResult<IteratorTp, OpTp>::IterateTaskWithResult(const parframe::range1d<iterator_t>& range, Args&... args)
    :
 task_(range, args...)
{}
