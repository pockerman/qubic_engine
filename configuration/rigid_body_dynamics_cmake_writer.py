...
class RBDynamicsCMakeWriter(CMakeFileWriter):

    @staticmethod
    def dir_path():
        current_dir = Path(os.getcwd())
        return current_dir / "kernel" / "rigid_body_dynamics"
    ...
    def __init__(self, configuration: dict, kernel_dirs: list, kernel_dir: Path) -> None:
        super(RBDynamicsCMakeWriter, self).__init__(configuration=configuration,
                                                    project_name="kernel_rigid_body_dynamics",
                                                    install_prefix=configuration["kernel"]["CMAKE_INSTALL_PREFIX"])

        self.kernel_dirs = kernel_dirs
        self.kernel_dir = kernel_dir
        self.dirs = RBDynamicsCMakeWriter.module_dirs()
    ...
    def write_cmake_lists(self):
        ...
        with open("kernel/rigid_body_dynamics/CMakeLists.txt", 'w', newline="\n") as fh:
            ...
            # include and library directoriers for Trilinos
            ...
            src_dir = current_dir / "kernel" / "rigid_body_dynamics" / "src" / "kernel" / "dynamics"
    ...
    def _write_single_cmake(self, path: Path, directory: str, example: bool) -> None:
        ...
        with open(path / "CMakeLists.txt", "w", newline="\n") as tfh:
            ...
            tfh.write('INCLUDE_DIRECTORIES({0})\n'.format(current_dir / "kernel" / "rigid_body_dynamics" / "src"))
    ...
    def _write_examples_cmake(self):
        ...
        # get the test directories
        examples_path = current_dir / "kernel/rigid_body_dynamics/examples"
    ...
    def _write_tests_cmake(self):
        ...
        # get the test directories
        path = current_dir / "kernel/rigid_body_dynamics/tests"
