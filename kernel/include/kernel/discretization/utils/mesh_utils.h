...
template<typename MeshTp>
const typename MeshTp::element_t*
find_first_closest_element(const MeshTp& mesh, const GeomPoint<MeshTp::dimension>& p, uint_t nsamples, real_t tol){
...
find_closest_point_to(const MeshTp& mesh, const GeomPoint<2>& p,
    ...
    for(; begin != end; ++begin){
        ...
        if(std::fabs(v1[0] - v0[0]) < tol){
            ...
            for(uint_t sp = 1; sp <=nsamples; ++sp){
                ...
                real_t xi = v0[1];
                points_on_segment[sp] = point_t({v0[0] + sp*h, });
            }
    ...
    auto element = find_first_closest_element(mesh, result, tol);
