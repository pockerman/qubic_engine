...
TEST(TestMeshTopology, TestDefaultConstuction) {

    /***
       * Test Scenario:    The application attempts to instantiate and empty toplogy
       * Expected Output:  MeshTopology instance should be generated
     **/

    using kernel::uint_t;
    using kernel::numerics::MeshTopology;

   MeshTopology<2> topology;
   ASSERT_EQ(topology.n_nodes(), 0);
   ASSERT_EQ(topology.n_edges(), 0);
   ASSERT_EQ(topology.n_faces(), 0);
   ASSERT_EQ(topology.n_elements(), 0);
...
TEST(TestMeshTopology, TestEqualEndPoints) {
...
TEST(TestMeshTopology, TestMeshGeneration) {
